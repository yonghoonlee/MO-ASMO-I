...
function ObjectiveFunction = obj(x,param)
    % Plot control
    ...
    while (t<=T)
        ...
        % YHL PLOT
        subplot(3,1,1);
        semilogy(k,max(abs(dUrdt)),'r.'); hold on;
        semilogy(k,max(abs(dUthdt)),'g.');
        semilogy(k,max(abs(dUzdt)),'b.');
        semilogy(k,max(abs(dTrrdt)),'k.');
        semilogy(k,max(abs(dTrtdt)),'rx');
        semilogy(k,max(abs(dTrzdt)),'gx');
        semilogy(k,max(abs(dTttdt)),'bx');
        semilogy(k,max(abs(dTtzdt)),'kx');
        semilogy(k,max(abs(dTzzdt)),'r*');
        if (k>=700)
        ...
        subplot(3,1,2);
        semilogy(k,M,'r.'); hold on;
        subplot(3,1,3);
        semilogy(k,Fn,'b.'); hold on;
        end
        if (mod(k,100) == 0)
            drawnow;
        end
...
%         if k == 3
%             dUrdtINIT = max(max(abs(dUrdt))) + eps;
%             dUthdtINIT = max(max(abs(dUthdt))) + eps;
%             dUzdtINIT = max(max(abs(dUzdt))) + eps;
%             dTrrdtINIT = max(max(abs(dTrrdt))) + eps;
%             dTrtdtINIT = max(max(abs(dTrtdt))) + eps;
%             dTrzdtINIT = max(max(abs(dTrzdt))) + eps;
%             dTttdtINIT = max(max(abs(dTttdt))) + eps;
%             dTtzdtINIT = max(max(abs(dTtzdt))) + eps;
%             dTzzdtINIT = max(max(abs(dTzzdt))) + eps;
%             MINIT = M + eps;
%             FnINIT = Fn + eps;
%         end
        %stop_criteria1 = max(max(abs([dUrdt,dUthdt,dUzdt,dTrrdt,dTrtdt,dTrzdt,dTttdt,dTtzdt,dTzzdt])));
        if (k>10)
%             stop_criteria3 = max(max(abs([dUrdt/dUrdtINIT,dUthdt/dUthdtINIT,...
%                                           dUzdt/dUzdtINIT,dTrrdt/dTrrdtINIT,...
%                                           dTrtdt/dTrtdtINIT,dTrzdt/dTrzdtINIT,...
%                                           dTttdt/dTttdtINIT,dTtzdt/dTtzdtINIT,...
%                                           dTzzdt/dTzzdtINIT])));
%             if (stop_criteria3<1e-2)
%                 break;
%             end
        ...
        if plotting
            ...
            if (k>10)
                subplot(3,1,3);
                semilogy(k,stop_criteria5,'rs'); hold on;
                semilogy(k,stop_criteria6,'bd'); hold on;
                semilogy(k,stop_criteria,'k.'); hold on;
            end
        end
